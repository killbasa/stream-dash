<script lang="ts">
	...
	const handleDelete = async (id: string) => {
		const response = await fetch(`/api/${params.orgSlug}/live-inputs/${id}`, {
			method: 'DELETE',
		});
	...
	const handleSync = async () => {
		const response = await fetch(`/api/${params.orgSlug}/live-inputs/sync`, {
			method: 'POST',
		});
...
<svelte:head>
	<title>Live Inputs | {params.orgSlug}</title>
</svelte:head>
					...
					<TableBodyRow>
						...
						<TableBodyCell>
							<Button
								size="xs"
								color="alternative"
								href="/{params.orgSlug}/live-inputs/{entry.id}">View</Button
							>
							<Button
								type="button"
