<script lang="ts">
	...
	const handleDelete = async (id: string) => {
		const response = await fetch(`/api/${params.orgSlug}/blocks/${id}`, {
			method: 'DELETE',
		});
...
<svelte:head>
	<title>Blocks | {params.orgSlug}</title>
</svelte:head>
...
<Container>
	...
	<div>
		<Button href="/{params.orgSlug}/blocks/new" class="cursor-pointer" size="xs">Create</Button>
	</div>
					...
					<TableBodyRow>
						...
						<TableBodyCell>
							<div class="flex flex-col gap-1">
								{#each entry.talents as talent (talent.id)}
									<a
										class="underline"
										href="/{params.orgSlug}/talents/{talent.id}"
										>{talent.name}</a
									>
								{/each}
							</div>
						...
						<TableBodyCell>
							{#if entry.location}
								<a
									class="underline"
									href="/{params.orgSlug}/locations/{entry.location.id}"
						...
						<TableBodyCell>
							<Button
								href="/{params.orgSlug}/blocks/{entry.id}"
								size="xs"
								color="alternative"
								class="cursor-pointer">View</Button
							>
							<Button
								href="/{params.orgSlug}/blocks/{entry.id}/edit"
								size="xs"
								color="alternative"
