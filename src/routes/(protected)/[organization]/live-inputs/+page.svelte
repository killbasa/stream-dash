<script lang="ts">
	...
	const handleDelete = async (id: string) => {
		const response = await fetch(`/api/${params.organization}/live-inputs/${id}`, {
			method: 'DELETE',
		});
	...
	const handleSync = async () => {
		const response = await fetch(`/api/${params.organization}/live-inputs/sync`, {
			method: 'POST',
		});
...
<svelte:head>
	<title>Live Inputs | {params.organization}</title>
</svelte:head>
