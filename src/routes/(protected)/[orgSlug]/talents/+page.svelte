<script lang="ts">
	...
	const handleDelete = async (id: string) => {
		const response = await fetch(`/api/${params.orgSlug}/talents/${id}`, {
			method: 'DELETE',
		});
...
<svelte:head>
	<title>Talents | {params.orgSlug}</title>
</svelte:head>
					...
					<TableBodyRow>
						...
						<TableBodyCell>
							<Button
								size="xs"
								color="alternative"
								href="/{params.orgSlug}/talents/{entry.id}">View</Button
							>
							<Button
								size="xs"
								color="alternative"
								href="/{params.orgSlug}/blocks?talent={entry.id}">Blocks</Button
							>
							<Button
								type="button"
